...
macroAwareEqualitySpec = describe "macro-aware equality (key optimization)" $ do
  ...
  it "expands macro vs non-macro comparisons" $ do
    let env = noMacros
        env' = extendMacroEnvironment "Id" [] (RelMacro (Arr (RVar "X" (-1) (initialPos "test")) (RVar "X" (-1) (initialPos "test")) (initialPos "test"))) (defaultFixity "ID") env
...
errorConditionSpec = describe "error conditions" $ do
  it "reports macro arity mismatch" $ do
    let env = noMacros
        env' = extendMacroEnvironment "Pair" ["A", "B"] (RelMacro (Comp (RVar "A" 1 (initialPos "test")) (RVar "B" 0 (initialPos "test")) (initialPos "test"))) (defaultFixity "ID") env
...
typeOpsErrorEdgeCasesSpec = describe "type operations error edge cases" $ do
  it "handles deeply nested macro expansion errors" $ do
    ...
    let env = noMacros
        env1 = extendMacroEnvironment "A" [] (RelMacro (RMacro "B" [] (initialPos "test"))) (defaultFixity "ID") env
  ...
  it "handles macro arity mismatches with complex arguments" $ do
    ...
    let env = noMacros
        env' = extendMacroEnvironment "Binary" ["A", "B"] (RelMacro (Comp (RVar "A" 1 (initialPos "test")) (RVar "B" 0 (initialPos "test")) (initialPos "test"))) (defaultFixity "Binary") env
