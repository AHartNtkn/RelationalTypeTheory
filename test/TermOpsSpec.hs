...
termExpansionSpec = describe "TMacro expansion" $ do
  it "expands simple TMacro with no parameters" $ do
    let macroEnv = extendMacroEnvironment "true" [] (TermMacro (Lam "x" (Lam "y" (Var "x" 1 (initialPos "test")) (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "expands TMacro with single parameter" $ do
    let macroEnv = extendMacroEnvironment "id" ["x"] (TermMacro (Var "x" 0 (initialPos "test"))) noMacros
  ...
  it "expands TMacro with multiple parameters" $ do
    let macroEnv = extendMacroEnvironment "app" ["f", "x"] (TermMacro (App (Var "f" 1 (initialPos "test")) (Var "x" 0 (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "expands nested TMacros" $ do
    let macroEnv =
          extendMacroEnvironment "id" ["x"] (TermMacro (Var "x" 0 (initialPos "test"))) $
            extendMacroEnvironment "twice" ["f", "x"] (TermMacro (App (Var "f" 1 (initialPos "test")) (App (Var "f" 1 (initialPos "test")) (Var "x" 0 (initialPos "test")) (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "expands TMacros in complex terms" $ do
    let macroEnv = extendMacroEnvironment "const" ["x", "y"] (TermMacro (Var "x" 1 (initialPos "test"))) noMacros
...
termExpansionErrorSpec = describe "TMacro expansion errors" $ do
  ...
  it "fails on macro arity mismatch - too few arguments" $ do
    let macroEnv = extendMacroEnvironment "binary" ["x", "y"] (TermMacro (App (Var "x" 1 (initialPos "test")) (Var "y" 0 (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "fails on macro arity mismatch - too many arguments" $ do
    let macroEnv = extendMacroEnvironment "unary" ["x"] (TermMacro (Var "x" 0 (initialPos "test"))) noMacros
  ...
  it "fails when trying to use relational macro as term macro" $ do
    let macroEnv = extendMacroEnvironment "relMacro" ["X"] (RelMacro (RVar "X" 0 (initialPos "test"))) noMacros
...
termExpansionEdgeCasesSpec = describe "TMacro expansion edge cases" $ do
  it "handles deeply nested macro expansions within step limit" $ do
    ...
    let macroEnv = extendMacroEnvironment "wrap" ["x"] (TermMacro (Lam "f" (App (Var "f" 0 (initialPos "test")) (Var "x" 1 (initialPos "test")) (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "handles TMacros with complex argument expressions" $ do
    let macroEnv = extendMacroEnvironment "apply" ["f", "x"] (TermMacro (App (Var "f" 1 (initialPos "test")) (Var "x" 0 (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "preserves variable scoping during expansion" $ do
    let macroEnv = extendMacroEnvironment "test" ["x"] (TermMacro (Lam "y" (App (Var "x" 1 (initialPos "test")) (Var "y" 0 (initialPos "test")) (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "handles empty argument lists correctly" $ do
    let macroEnv = extendMacroEnvironment "unit" [] (TermMacro (Lam "x" (Var "x" 0 (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "correctly substitutes multiple occurrences of parameters" $ do
    let macroEnv = extendMacroEnvironment "dup" ["x"] (TermMacro (App (Var "x" 0 (initialPos "test")) (Var "x" 0 (initialPos "test")) (initialPos "test"))) noMacros
  ...
  it "expands term macros within promotions in relational contexts" $ do
    let macroEnv = extendMacroEnvironment "Id" [] (TermMacro (Lam "x" (Var "x" 0 (initialPos "test")) (initialPos "test"))) noMacros
