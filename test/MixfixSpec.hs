...
import Lib
import Parser (ParseContext (..), Parser, emptyParseContext, parseDeclaration, parseFile, parseTerm, parseRType, runParserEmpty, runParserT, mixfixIdentifier)
...
mixfixMacroDefSpec = describe "Mixfix macro definition parsing" $ do
  ...
  it "parses ternary mixfix macros with auto-parameters" $ do
    testParseDecl "if_then_else_ c t e := t;" 
      (MacroDef "if_then_else_" ["c", "t", "e"] 
...
mixfixParsingSpec = describe "Mixfix expression parsing" $ do
  ...
  it "parses ternary mixfix expressions" $ do
    ...
    testParseExpr env ["c", "t", "e"] "if c then t else e"
