...
spec = do
  ...
  describe "Term Pretty Printing" $ do
    describe "basic terms" $ do
      it "pretty prints variables" $ do
        prettyDefault (Var "x" 0 (initialPos "test")) `shouldBe` "x"

      it "pretty prints variables with indices when configured" $ do
        let config = defaultPrettyConfig {showIndices = True}
        prettyWithConfig config (Var "x" 0 (initialPos "test")) `shouldBe` "x_0"
        prettyWithConfig config (Var "y" 1 (initialPos "test")) `shouldBe` "y_1"

      it "pretty prints lambda abstractions with Unicode" $ do
        prettyDefault (Lam "x" (Var "x" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "λ x . x"

      it "pretty prints lambda abstractions with ASCII" $ do
        let config = defaultPrettyConfig {useUnicode = False}
        prettyWithConfig config (Lam "x" (Var "x" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "\\x . x"

      it "pretty prints applications" $ do
        prettyDefault (App (Var "f" 0 (initialPos "test")) (Var "x" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "f x"
    ...
    describe "complex terms" $ do
      it "pretty prints nested lambda abstractions" $ do
        ...
        prettyDefault term `shouldBe` "λ x . λ y . x"
      ...
      it "pretty prints nested applications" $ do
        ...
        prettyDefault term `shouldBe` "f x y"
      ...
      it "adds parentheses for complex applications" $ do
        ...
        prettyDefault term `shouldBe` "(λ x . x) y"
      ...
      it "handles deeply nested terms" $ do
        ...
        prettyDefault term `shouldBe` "λ f . λ x . f (f x)"

  describe "Type Pretty Printing" $ do
    describe "basic types" $ do
      it "pretty prints type variables" $ do
        prettyDefault (RVar "X" 0 (initialPos "test")) `shouldBe` "X"

      it "pretty prints type applications" $ do
        prettyDefault (RMacro "List" [MRel (RVar "X" 0 (initialPos "test"))] (initialPos "test")) `shouldBe` "List X"
        prettyDefault (RMacro "Map" [MRel (RVar "K" 0 (initialPos "test")), MRel (RVar "V" 0 (initialPos "test"))] (initialPos "test")) `shouldBe` "Map K V"

      it "pretty prints arrow types with Unicode" $ do
        prettyDefault (Arr (RVar "X" 0 (initialPos "test")) (RVar "Y" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "X → Y"

      it "pretty prints arrow types with ASCII" $ do
        let config = defaultPrettyConfig {useUnicode = False}
        prettyWithConfig config (Arr (RVar "X" 0 (initialPos "test")) (RVar "Y" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "X -> Y"

      it "pretty prints universal quantification with Unicode" $ do
        prettyDefault (All "X" (RVar "X" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "∀ X . X"

      it "pretty prints universal quantification with ASCII" $ do
        let config = defaultPrettyConfig {useUnicode = False}
        prettyWithConfig config (All "X" (RVar "X" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "forallX . X"

    describe "relational operations" $ do
      it "pretty prints composition with Unicode" $ do
        prettyDefault (Comp (RVar "R" 0 (initialPos "test")) (RVar "S" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "R ∘ S"

      it "pretty prints composition with ASCII" $ do
        let config = defaultPrettyConfig {useUnicode = False}
        prettyWithConfig config (Comp (RVar "R" 0 (initialPos "test")) (RVar "S" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "R o S"

      it "pretty prints converse with Unicode" $ do
        prettyDefault (Conv (RVar "R" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "R ˘"

      it "pretty prints converse with ASCII" $ do
        let config = defaultPrettyConfig {useUnicode = False}
        prettyWithConfig config (Conv (RVar "R" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "R^"

      it "pretty prints promotion (hides Prom from user)" $ do
        prettyDefault (Prom (Lam "x" (Var "x" 0 (initialPos "test")) (initialPos "test")) (initialPos "test")) `shouldBe` "λ x . x"

      it "hides promotion for term macros" $ do
        prettyDefault (Prom (TMacro "Id" [] (initialPos "test")) (initialPos "test")) `shouldBe` "Id"

      it "hides promotion for complex terms" $ do
        prettyDefault (Prom (App (Var "f" 0 (initialPos "test")) (Var "x" 0 (initialPos "test")) (initialPos "test")) (initialPos "test")) `shouldBe` "f x"

      it "hides promotion in nested contexts" $ do
        prettyDefault (Comp (Prom (Var "f" 0 (initialPos "test")) (initialPos "test")) (RVar "R" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "f ∘ R"
    ...
    describe "advanced relational macro applications" $ do
      it "pretty prints zero-argument relational macros" $ do
        prettyDefault (RMacro "Unit" [] (initialPos "test")) `shouldBe` "Unit"
        prettyDefault (RMacro "Bottom" [] (initialPos "test")) `shouldBe` "Bottom"
      ...
      it "pretty prints nested relational macro applications" $ do
        ...
        prettyDefault nested `shouldBe` "Outer Inner X Y"
      ...
      it "pretty prints relational macros with complex type arguments" $ do
        ...
        prettyDefault complex `shouldBe` "Transform (A → B) (∀ X . X)"
      ...
      it "pretty prints relational macros with relational operations" $ do
        ...
        prettyDefault withOps `shouldBe` "Compose (R ∘ S) T˘"
    ...
    describe "complex types" $ do
      it "pretty prints nested arrow types" $ do
        ...
        prettyDefault rtype `shouldBe` "X → Y → Z"
      ...
      it "pretty prints nested quantifiers" $ do
        ...
        prettyDefault rtype `shouldBe` "∀ X . ∀ Y . X → Y"
      ...
      it "pretty prints complex composition chains" $ do
        ...
        prettyDefault rtype `shouldBe` "(R ∘ S) ∘ T"
  ...
  describe "Proof Pretty Printing" $ do
    describe "basic proofs" $ do
      ...
      it "pretty prints proof lambda abstractions" $ do
        ...
        prettyDefault proof `shouldBe` "λ u : R . p"

      it "pretty prints proof applications" $ do
        prettyDefault (AppP (PVar "p" 0 (initialPos "test")) (PVar "q" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "p q"

      it "pretty prints type lambda abstractions with Unicode" $ do
        prettyDefault (TyLam "X" (PVar "p" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "Λ X . p"

      it "pretty prints type lambda abstractions with ASCII" $ do
        let config = defaultPrettyConfig {useUnicode = False}
        prettyWithConfig config (TyLam "X" (PVar "p" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "LambdaX . p"

      it "pretty prints type applications" $ do
        prettyDefault (TyApp (PVar "p" 0 (initialPos "test")) (RVar "X" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "p { X }"
    ...
    describe "advanced proof constructs" $ do
      it "pretty prints iota (term promotion introduction)" $ do
        ...
        prettyDefault proof `shouldBe` "ι⟨λ x . x, λ y . y⟩"
      ...
      it "pretty prints conversion proofs" $ do
        ...
        prettyDefault proof `shouldBe` "x ⇃ p ⇂ y"
      ...
      it "pretty prints rho elimination" $ do
        ...
        prettyDefault proof `shouldBe` "ρ{ x .t1, t2} p - q"

      it "pretty prints pi elimination" $ do
        let proof = Pi (PVar "p" 0 (initialPos "test")) "x" "u" "v" (PVar "q" 0 (initialPos "test")) (initialPos "test")
        prettyDefault proof `shouldBe` "π p - x . u . v .q"

      it "pretty prints converse introduction with Unicode" $ do
        prettyDefault (ConvIntro (PVar "p" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "∪ᵢ p"

      it "pretty prints converse introduction with ASCII" $ do
        let config = defaultPrettyConfig {useUnicode = False}
        prettyWithConfig config (ConvIntro (PVar "p" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "unionI p"

      it "pretty prints converse elimination with Unicode" $ do
        prettyDefault (ConvElim (PVar "p" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "∪ₑ p"

      it "pretty prints converse elimination with ASCII" $ do
        let config = defaultPrettyConfig {useUnicode = False}
        prettyWithConfig config (ConvElim (PVar "p" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "unionE p"

      it "pretty prints pairs" $ do
        prettyDefault (Pair (PVar "p" 0 (initialPos "test")) (PVar "q" 0 (initialPos "test")) (initialPos "test")) `shouldBe` "(p, q)"
      ...
      it "pretty prints complex rho elimination with nested terms" $ do
        ...
        prettyDefault proof `shouldBe` "ρ{ x . λ y . y, f a} ∪ᵢ p - ∪ₑ q"
      ...
      it "pretty prints complex pi elimination with multiple bindings" $ do
        ...
        prettyDefault proof `shouldBe` "π p { X } - x . u . v .(q, r)"
      ...
      it "pretty prints nested proof applications with parentheses" $ do
        ...
        prettyDefault proof `shouldBe` "(f ∪ᵢ p) ∪ₑ q"
      ...
      it "pretty prints iota with complex terms" $ do
        ...
        prettyDefault proof `shouldBe` "ι⟨(λ x . x) y, λ f . λ x . f x⟩"
      ...
      it "pretty prints conversion proofs with complex terms" $ do
        ...
        prettyDefault proof `shouldBe` "f x ⇃ Λ X . p ⇂ λ y . g y"
    ...
    describe "complex proofs" $ do
      it "adds parentheses for complex proof applications" $ do
        ...
        prettyDefault proof `shouldBe` "(λ u : R . p) q"
      ...
      it "handles nested proof lambda abstractions" $ do
        ...
        prettyDefault proof `shouldBe` "λ u : R . λv:S. p"
      ...
      it "handles deeply nested type applications" $ do
        ...
        prettyDefault proof `shouldBe` "p { X }{Y}"
  ...
  describe "Error Pretty Printing" $ do
    ...
    describe "variable and binding errors" $ do
      it "pretty prints unbound variable errors" $ do
        ...
        formatError err `shouldContainErrorMessage` "Unbound variable: x"

      it "pretty prints unbound type variable errors" $ do
        let err = UnboundTypeVariable "X" noCtx
        formatError err `shouldContainErrorMessage` "Unbound type variable: X"

      it "pretty prints unbound macro errors" $ do
        let err = UnboundMacro "MyMacro" noCtx
        formatError err `shouldContainErrorMessage` "Unbound macro: MyMacro"

      it "pretty prints duplicate binding errors" $ do
        let err = DuplicateBinding "x" noCtx
        formatError err `shouldContainErrorMessage` "Duplicate binding: x"
    ...
    describe "type system errors" $ do
      it "pretty prints type mismatch errors" $ do
        ...
        formatError err `shouldContain` "Type mismatch:"
        formatError err `shouldContain` "Expected: X"
        formatError err `shouldContain` "Actual: Y"

      it "pretty prints invalid type application errors" $ do
        let err = InvalidTypeApplication (RVar "X" 0 (initialPos "test")) noCtx
        formatError err `shouldContainErrorMessage` "Invalid type application: X"

      it "pretty prints macro arity mismatch errors" $ do
        let err = MacroArityMismatch "MyMacro" 2 1 noCtx
        formatError err `shouldContain` "Macro arity mismatch for MyMacro:"
        formatError err `shouldContain` "Expected: 2 arguments"
        formatError err `shouldContain` "Actual: 1 arguments"
    ...
    describe "proof checking errors" $ do
      it "pretty prints proof typing errors" $ do
        ...
        formatError err `shouldContain` "Proof error:"
        formatError err `shouldContain` "Expected judgment: x [R] y"
        formatError err `shouldContain` "Actual judgment: x [S] y"
      ...
      it "pretty prints left conversion errors" $ do
        ...
        formatError err `shouldContain` "expected x but got x'"
        formatError err `shouldContain` "β-η equivalent"
      ...
      it "pretty prints right conversion errors" $ do
        ...
        formatError err `shouldContain` "expected y but got y'"
        formatError err `shouldContain` "β-η equivalent"
      ...
      it "pretty prints converse errors" $ do
        ...
        formatError err `shouldContainErrorMessage` "Converse elimination error: proof dummy must prove judgment with converse relation, but proves x [X] y"
      ...
      it "pretty prints rho elimination errors" $ do
        ...
        formatError err `shouldContainErrorMessage` "Rho elimination error: first proof dummy must prove a judgment with promoted relation, but proves x [X] y"
  ...
  describe "Comprehensive Integration Tests" $ do
    it "pretty prints complex boolean type definition" $ do
      ...
      prettyDefault boolType `shouldBe` "∀ X . X → X → X"
    ...
    it "pretty prints complex proof with multiple constructs" $ do
      ...
      prettyDefault complexProof `shouldBe` "Λ X . λ p : R . ∪ₑ p"
    ...
    it "maintains consistency between Unicode and ASCII modes" $ do
      ...
      prettyDefault term `shouldContain` "λ"
      prettyDefault rtype `shouldContain` "∀"
      prettyDefault rtype `shouldContain` "→"
      prettyDefault proof `shouldContain` "Λ"
      prettyDefault proof `shouldContain` "∪ᵢ"

      -- ASCII should use text
      let asciiConfig = defaultPrettyConfig {useUnicode = False}
      prettyWithConfig asciiConfig term `shouldContain` "\\"
      prettyWithConfig asciiConfig rtype `shouldContain` "forall"
      prettyWithConfig asciiConfig rtype `shouldContain` "->"
      prettyWithConfig asciiConfig proof `shouldContain` "Lambda"
      prettyWithConfig asciiConfig proof `shouldContain` "unionI"
    ...
    it "handles all relational operations correctly" $ do
      ...
      prettyDefault composition `shouldBe` "R ∘ S"
      prettyDefault converse `shouldBe` "R ˘"
      prettyDefault promotion `shouldBe` "λ x . x"
    ...
    it "handles complex nested structures with proper parenthesization" $ do
      let complexType =
            Arr
              ...
              (initialPos "test")
      prettyDefault complexType `shouldBe` "(∀ X . X → X) → R ˘ ∘ S"
    ...
    it "preserves de Bruijn index information when requested" $ do
      ...
      prettyWithConfig config term `shouldBe` "λ x . λ y . x_1 y_0"

  describe "Term Macro (TMacro) Pretty Printing" $ do
    it "pretty prints zero-arity term macros" $ do
      prettyDefault (TMacro "Id" [] (initialPos "test")) `shouldBe` "Id"

    it "pretty prints term macros with single argument" $ do
      prettyDefault (TMacro "Apply" [MTerm (Var "f" 0 (initialPos "test"))] (initialPos "test")) `shouldBe` "Apply f"

    it "pretty prints term macros with multiple arguments" $ do
      prettyDefault (TMacro "Comp" [MTerm (Var "f" 0 (initialPos "test")), MTerm (Var "g" 0 (initialPos "test"))] (initialPos "test")) `shouldBe` "Comp f g"
    ...
    it "pretty prints term macros with complex arguments" $ do
      ...
      prettyDefault complex `shouldBe` "Map (λ x . x) (f a)"
    ...
    it "pretty prints term macros with lambda arguments requiring parentheses" $ do
      ...
      prettyDefault withLam `shouldBe` "Apply (λ x . x y)"

    it "handles empty argument lists consistently" $ do
      prettyDefault (TMacro "Nil" [] (initialPos "test")) `shouldBe` "Nil"
      prettyDefault (TMacro "Unit" [] (initialPos "test")) `shouldBe` "Unit"
  ...
  describe "Configuration Edge Cases and Pathological Tests" $ do
    it "handles empty and special names" $ do
      ...
      prettyDefault (Var "x'" 0 (initialPos "test")) `shouldBe` "x'"
      prettyDefault (Var "x_123" 0 (initialPos "test")) `shouldBe` "x_123"
      prettyDefault (RVar "R'" 0 (initialPos "test")) `shouldBe` "R'"

    it "handles very long names gracefully" $ do
      let longName = "very_very_very_long_variable_name_that_could_cause_issues"
      prettyDefault (Var longName 0 (initialPos "test")) `shouldBe` longName
      prettyDefault (RVar longName 0 (initialPos "test")) `shouldBe` longName
    ...
    it "handles deep nesting without stack overflow" $ do
      ...
      let result = prettyDefault deepNested
    ...
    it "handles deeply nested type applications" $ do
      ...
      prettyDefault deepType `shouldBe` "F G H X"
    ...
    it "handles large argument lists" $ do
      ...
      let result = prettyDefault manyArgMacro
    ...
    it "preserves Unicode vs ASCII consistency across all constructs" $ do
      ...
      prettyWithConfig unicodeConfig term `shouldContain` "λ"
      prettyWithConfig asciiConfig term `shouldContain` "\\"

      prettyWithConfig unicodeConfig rtype `shouldContain` "∀"
      prettyWithConfig asciiConfig rtype `shouldContain` "forall"

      prettyWithConfig unicodeConfig proof `shouldContain` "Λ"
      prettyWithConfig asciiConfig proof `shouldContain` "Lambda"

    it "handles showIndices configuration for all construct types" $ do
      let config = defaultPrettyConfig {showIndices = True}

      prettyWithConfig config (Var "x" 5 (initialPos "test")) `shouldBe` "x_5"
      prettyWithConfig config (RVar "R" 3 (initialPos "test")) `shouldBe` "R_3"
      prettyWithConfig config (PVar "p" 2 (initialPos "test")) `shouldBe` "p_2"
    ...
    it "handles complex mixed constructs" $ do
      ...
      let complex =
            TMacro
              ...
              (initialPos "test")
      let result = prettyDefault complex
    ...
    it "handles pathological parenthesization cases" $ do
      ...
      prettyDefault nestedApp `shouldBe` "(λ f . λ x . f) g h"
      ...
      prettyDefault nestedRType `shouldBe` "(A → B) → C → D"
